 ...
 */
 ...
 export default function App(props) {
    ...
    const [patient, setPatient] = useState(props.patientName);
  
    const getTableDatas = async () => {
      try {
       const response = await fetch('http://'+props.api_url+'/api/patients');
  ...
  function table(data) {
    return (
      <View style={styles.table}>
            ...
            data["hydra:member"].map((element, index) => {
              return (
                <DataTable.Row key={index}>
                  ...
                  <DataTable.Cell>
                  <IconButton style={styles.btnSelect} onPress={() => {setPatient(element)}} icon={props => <Icon name="select" {...props} />} />
                  </DataTable.Cell>
                </DataTable.Row>
              )
            })
        ...
        <View style={styles.validationBar}>
          ...
          <Pressable onPress={() => {props.onPageChange('main'); props.onPatientChange(patient)}} style={styles.btnValidate}>
            <Text>
              Valider
            </Text>
          </Pressable>
    ...
    return (
      <View style={styles.container}>
        <Text>Choix du patient :</Text>
        <Text>
            {patient==null ? 'Non sélectionner' : patient.nom + ' ' + patient.prenom}
        </Text>
